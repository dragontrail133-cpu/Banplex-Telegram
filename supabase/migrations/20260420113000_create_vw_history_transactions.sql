...
as
  select
    ...
    pi.id,
    coalesce(
      pi.transaction_date::timestamptz,
      pi.income_date::timestamptz,
      pi.created_at
    ) as sort_at,
    pi.transaction_date,
    pi.income_date,
  ...
  select
    ...
    l.id,
    coalesce(
      l.transaction_date::timestamptz,
      l.disbursed_date::timestamptz,
      l.created_at
    ) as sort_at,
    l.transaction_date,
    null::date as income_date,
...
create or replace view public.vw_history_transactions
with (security_invoker = true)
as
select *
from public.vw_workspace_transactions
where (
  source_type in ('project-income', 'expense')
  and (
    bill_id is null
    or bill_status = 'paid'
    or bill_remaining_amount <= 0
  )
)
or (
...
grant select on table public.vw_history_transactions to anon, authenticated;
