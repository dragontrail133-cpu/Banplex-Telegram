...
test('aggregateProjectIncomeViewRows collapses duplicate income fee-bill rows', () => {
  ...
  assert.ok(incomeRow)
  assert.equal(incomeRow.bill_count, 0)
  assert.deepEqual(incomeRow.bill_ids, [])
  assert.equal(incomeRow.bill_amount, null)
  assert.equal(incomeRow.bill_paid_amount, null)
  assert.equal(incomeRow.bill_remaining_amount, null)
  assert.equal(incomeRow.bill_status, null)
  assert.equal(incomeRow.bill_paid_at, null)
  assert.equal(incomeRow.bill_due_date, null)
  assert.equal(incomeRow.bill_description, null)
  assert.equal(incomeRow.bill_worker_name_snapshot, null)
  assert.equal('fee_bill_ids' in incomeRow, false)
  assert.equal('fee_bill_count' in incomeRow, false)
  assert.equal('fee_bill_amount' in incomeRow, false)
  assert.equal('fee_bill_paid_amount' in incomeRow, false)
