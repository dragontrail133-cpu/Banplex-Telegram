...
test('aggregateProjectIncomeViewRows collapses duplicate income fee-bill rows', () => {
  ...
  assert.deepEqual(incomeRow.fee_bill_ids, ['bill-1', 'bill-2'])
  assert.equal(incomeRow.fee_bill_count, 2)
  assert.equal(incomeRow.fee_bill_amount, 750000)
  assert.equal(incomeRow.fee_bill_paid_amount, 100000)
