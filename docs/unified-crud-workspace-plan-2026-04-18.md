...
- [x] UCW-367 - Bereskan audit UI pasca-backfill untuk nominal attendance, Payroll history, dan PDF faktur/proyek
  - Temuan audit UI setelah live backfill `.env` harus ditutup di source of truth final: row `half_day` tidak boleh tampil `0`, `Payroll` harus bisa membuka bulan historis hasil backfill, dan PDF `Proyek` / `Faktur` harus membaca nominal legacy yang benar.
